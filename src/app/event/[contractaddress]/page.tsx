...
export default function EventPage() {
    ...
    const eventAddress = params?.contractaddress;
    ...
    const contract = getContract({
        client: client,
        chain: baseSepolia,
        address: eventAddress as string,
    });
                            ...
                            <div className="absolute right-0 top-1/2 transform -translate-y-1/2">
                                {isEditing && (
                                    <p className="px-3 py-1 bg-gray-500 text-white text-sm rounded-md inline-block mr-2">
                                        Status:  
                                        {Number(state) === 0 ? "Active" : 
                                        Number(state) === 1 ? "Successful" :
                                        Number(state) === 2 ? "Failed" : "Unknown"}
                                    </p>
                                )}
                                <button
...
const CreateEventModal = (
    ...
    const handleTierNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      // Limit the string length to 50 characters
    ...
    const handleTierAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        // Safely parse the input; default to 1 if invalid/empty
        let value = parseInt(e.target.value, 10);
        if (isNaN(value)) value = Number(MIN_TIER_AMOUNT);

        // Clamp the value between MIN_TIER_AMOUNT and MAX_TIER_AMOUNT
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center backdrop-blur-md">
          <div className="w-1/2 bg-slate-100 dark:bg-gray-800 p-6 rounded-md">
            ...
            <div className="flex flex-col">
              ...
              <input
                type="text"
                maxLength={MAX_TIER_NAME_LENGTH} // this helps in browsers that respect the maxLength attribute
                value={tierName}
              ...
              <input
                type="number"
                // Provide min and max attributes for browsers that respect these constraints
